...
import { GitHubKeysFetcher as IGitHubKeysFetcher, githubKeysFetcherContext } from '@mcp-magnet/dialog';

class GitHubKeysFetcher implements IGitHubKeysFetcher {
  ...
  async fetchGitHubKeys(username: string): Promise<string[]> {
    ...
    // キャッシュに格納して返却
    this.keyCache.set(username, result);
    return result;
  }
}

export function GithubKeysFetcherProvider({ children }: { children: React.ReactNode }) {
  const fetcher = new GitHubKeysFetcher();
